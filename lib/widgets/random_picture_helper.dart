...
class ImagePrefetcher {
  ...
  // Queue of prefetched image data
  final List<Map<String, dynamic>> _prefetchedImages = [];
    final Set<String> _recentlyShownImageIds = {};
  bool _isPrefetching = false;
  List<String>? _availableTopics;
  ...
  Map<String, dynamic>? getRandomPrefetchedImage() {
    ...
    final random = Random();
// Try to find a non-recently-shown image
    List<Map<String, dynamic>> eligibleImages = _prefetchedImages.where((img) {
      return !_recentlyShownImageIds.contains(img['id']);
    }).toList();
    
    // If all images are recently shown, use any of them
    if (eligibleImages.isEmpty) {
      eligibleImages = _prefetchedImages;
    }
   final index = random.nextInt(eligibleImages.length);
    final selectedImage = eligibleImages[index];
// Remove the selected image from prefetched list
    _prefetchedImages.removeWhere((img) => img['id'] == selectedImage['id']);
      
    // Add to recently shown images
    _recentlyShownImageIds.add(selectedImage['id']);
    if (_recentlyShownImageIds.length > 20) {
      _recentlyShownImageIds.remove(_recentlyShownImageIds.first);
    }
    // Start prefetching new images if we're running low
    if (_prefetchedImages.length < 3 && !_isPrefetching) {
      _prefetchMoreImages();
...
Future<void> showRandomPicture(BuildContext context) async {
  ...
  try {
   ...
   // If no prefetched image, fetch a new one
    ...
    // Improve topic randomization by shuffling the list first
    final topics = categories.map((category) => category['query'].toString()).toList();
    topics.shuffle(); // Shuffle to increase randomness
    final randomTopic = topics[random.nextInt(topics.length)];
    final timestamp = DateTime.now().millisecondsSinceEpoch;

    final response = await http.get(
      Uri.parse(
        'https://api.unsplash.com/photos/random?query=$randomTopic&client_id=${UnsplashService.accessKey}&_t=$timestamp',
      ),
    );
    final newImages = await UnsplashService.fetchImages(
         randomTopic,
